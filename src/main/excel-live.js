 ...
 * classeur écrit avant l'arrivée d'une colonne (« Couloir », « Type ») n'a pas la même
...
const { buildModelRows, NODE_COLS, LINK_COLS, typeDepuisTexte } = require("./excel");
...
function diagrammeDepuisTables(res, sheetName) {
  ...
  for (const r of res.noeuds.lignes) {
    ...
    nodes.push({
      ...
      kind: typeDepuisTexte(val(r, cn, "Type")),
  ...
  return {
    ...
    // Voir readDiagram : sans la colonne, on ne touche ni aux couloirs ni aux
    // types que l'app connaît déjà.
    hasLane: res.noeuds.entetes.map(String).includes("Couloir"),
    hasKind: res.noeuds.entetes.map(String).includes("Type")
  };
}
